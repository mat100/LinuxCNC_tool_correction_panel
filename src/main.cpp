...
Encoder Encoder0(A3,10);      //A,B Pin - Changed to working pins from backup
Encoder Encoder1(11,12);    //A,B Pin - Changed to working pins from backup
...
#define LCD_VARS
  ...
  LCDVar lcdVars[] = {
    LCD_FLOAT("Drik:  ", 0, 0, 3),      // Speed on row 0, column 0, 1 decimal place
    LCD_FLOAT("Vyska: ", 2, 0, 3)       // Feed on row 1, column 0, 2 decimal places
  };
...
void readEncoders(){
    if(QuadEncs>=1){
      #if QUADENCS >= 1
        long rawRead0 = Encoder0.read();
        EncCount[0] = rawRead0/QuadEncMp[0];
      #else
        #ifdef DEBUG
          Serial.println("DEBUG: QUADENCS < 1, Encoder0 not compiled");
        #endif
      #endif
    }
    if(QuadEncs>=2){
      #if QUADENCS >= 2
        long rawRead1 = Encoder1.read();
        EncCount[1] = rawRead1/QuadEncMp[1];
      #else
        #ifdef DEBUG
          Serial.println("DEBUG: QUADENCS < 2, Encoder1 not compiled");
        #endif
      #endif
    }
    ...
    for(int i=0; i<QuadEncs;i++){
      if(QuadEncSig[i]==2){        
        if(OldEncCount[i] != EncCount[i]){
          ...
          #ifdef DEBUG
            Serial.print("DEBUG: No change for encoder ");
            Serial.println(i);
          #endif
        }
      }
    ...
    #ifdef DEBUG
      Serial.println("DEBUG: readEncoders() finished");
      Serial.println("---");
    #endif
}
